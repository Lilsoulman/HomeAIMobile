...
echo ========================================
echo  NexusMind Staging Base Release
echo ========================================
...
set "LOG_FILE=%LOG_DIR%\%TIMESTAMP%-staging-base.log"

echo.
echo Log: %LOG_FILE%
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$started=$false; $code=1; try { Start-Transcript -LiteralPath '%LOG_FILE%' -Force | Out-Null; $started=$true; & '%~dp0scripts\build_android_base.ps1' -Artifact '%ARTIFACT%' %DRY_ARG%; $code=0 } catch { Write-Host ('[FAILED] ' + $_.Exception.Message); Write-Host ($_ | Out-String); $code=1 } finally { if ($started) { Stop-Transcript | Out-Null } }; exit $code"
