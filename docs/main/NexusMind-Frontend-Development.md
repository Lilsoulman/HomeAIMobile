...
> **当前基线：** V2.4（专家会话化）。本文只承接已确认的页面、交互、接口消费和验收；产品范围以产品总设计为准。
...
- `expert_workbench_page.dart`：专家对话框——历史对话框列表 + 新建对话框（选专家/连接器）+ 对话框内多轮对话；既有专家目录按 `planning`/`review`/`life` 分类组织，`life` 分类个人生活专家以对话框入口呈现；
...
`expert_workbench_page.dart` 是专家对话框：以「历史对话框列表 + 新建对话框」承载对某领域内容的遍历询问，不是自由闲聊，每条消息即一次可追溯的 Expert Run。

- **会话列表页**：历史对话框（标题、绑定的专家名、连接器名、最后消息时间）、新建对话框入口；运行记录保留只读入口。
- **新建对话框**：填写标题（可选，默认「新对话」）→ 选择专家（**基础/自建分组展示并标识来源；自建仅创建者本人可见**）→ 选择连接器（可选，已授权实例，如可画/飞书/钉钉，可用性受 Provider 发布节奏约束）→ 创建会话。
- **对话框页**：消息气泡（user/assistant）、输入框与发送按钮；发送创建关联会话的 Expert Run，轮询终态后追加为 assistant 消息；多轮上下文由后端按会话历史拼接承载，移动端不做本地上下文缓存，重新进入按会话 ID 恢复消息历史。
- **已移出移动端（由 PC 端承接）**：附件选择/上传、Action 确认、运行时间线、生成文件（PPT）下载——移动端不再呈现这些运行细节。
...
| Conversation / Message | `ConversationDto`（会话：标题/绑定专家/连接器/最后消息时间）、`ConversationMessageDto`（role/content/runId）；`IConversationRepository`：会话列表、新建、重命名、删除、消息历史（游标分页）与发送（发送复用 Expert Run 链路） |
| Expert / AgentRun | 专家目录（含来源判定：基础/自建，`scope=basic\|mine\|all`）、运行、事件、取消与重试 |
...
V2.3 还必须验证：L1 批量确认限制、L2/L3 逐项确认、幂等与页面重入恢复、家庭知识权限与冲突呈现、设备异常语义、通知深链权限边界、Dashboard 局部失败不遮蔽待确认事项、个人生活专家（life 分类详情/翻牌建议卡/行程日历同步确认/收藏可见性与权限），以及静态扫描确认 Flutter 不包含 MCP stdio/SQLite 客户端、MCP 同步令牌或敏感开关。V2.4 专家会话化还必须验证：会话列表/新建（标题、选专家区分基础与自建来源、选连接器）、消息发送与轮询追加、历史游标分页恢复、跨用户会话 404 呈现、无附件/Action 确认/时间线/下载残留入口，且移动端不缓存会话上下文。产品、信息架构、数据边界或跨端流程变更必须先更新产品总设计；UI Token 变化同步 UI 样式规范和主题实现，工程架构变化同步开发规范。

## 8. 文档联动

本文是产品总设计到前端实现的拆分层。开发计划只维护当前已完成和下一步，不能反向定义产品范围，也不得累计历史变更。当前 V2.3/V2.4 同步覆盖：统一五 Tab 命名、三级风险确认、家庭知识写入权限、管家动态与技术审计的边界、通知优先级与聚合规则、Phase 1 的质量门槛、个人生活专家（探店翻牌、行程规划与日历同步、个人偏好收藏的可见性与权限）、专家会话化（会话列表/新建对话框/纯对话/专家来源区分/运行细节移 PC 端）。
