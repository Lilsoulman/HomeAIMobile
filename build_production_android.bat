...
cd /d "%~dp0"
chcp 65001 >nul
title NexusMind Production Android Build

set "EXPECTED_BRANCH=release"
set "FLAVOR=production"
set "CONFIG_FILE=config/production.json"
...
if defined DIRTY_WORKTREE (
    echo [FAILED] Production builds require a clean Git worktree.
    pause
    exit /b 1
)

echo ========================================
echo  NexusMind Production Android Build
echo ========================================
echo  1. Build APK
echo  2. Build AAB
echo.
...
if "%MENU%"=="1" (
    ...
    set "SOURCE_ARTIFACT=build\app\outputs\flutter-apk\app-production-release.apk"
)
if "%MENU%"=="2" (
    set "BUILD_COMMAND=appbundle"
    set "ARTIFACT_EXT=aab"
    set "SOURCE_ARTIFACT=build\app\outputs\bundle\productionRelease\app-production-release.aab"
)
...
if "%ARTIFACT_EXT%"=="aab" if not exist "android\key.properties" (
    echo [FAILED] Production AAB requires android\key.properties and a release keystore.
    pause
    exit /b 1
)
if "%ARTIFACT_EXT%"=="apk" if not exist "android\key.properties" echo [WARNING] This local production APK will use debug signing.

set /p "VERSION_NAME=Enter versionName (for example 1.2.0): "
if not defined VERSION_NAME (
    echo [FAILED] versionName is required.
    pause
    exit /b 1
)
set "NEXUS_VERSION_INPUT=%VERSION_NAME%"
powershell.exe -NoProfile -Command "if ($env:NEXUS_VERSION_INPUT -match '^\d+\.\d+\.\d+$') { exit 0 } else { exit 1 }"
set "VERSION_RESULT=%ERRORLEVEL%"
set "NEXUS_VERSION_INPUT="
if not "%VERSION_RESULT%"=="0" (
    echo [FAILED] versionName must use x.y.z format.
    pause
    exit /b 1
)
...
set "LOG_FILE=%LOG_DIR%\%TIMESTAMP%-production-%ARTIFACT_EXT%.log"
